...
- (void)sendPingWithData:(NSData *)data numMessages:(NSUInteger)numMessages {
    int                     err;
    NSData *                payload;
    NSData *                packet;
    ssize_t                 bytesSent;
...
- (void)sendPingWithData:(NSData *)data {
    [self sendPingWithData:data numMessages:1];
}
...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber] ) {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(simplePing:didReceivePingResponsePacket:sequenceNumber:address:)] ) {
                [strongDelegate simplePing:self didReceivePingResponsePacket:packet sequenceNumber:sequenceNumber address: address];
            }
        } else {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(simplePing:didReceiveUnexpectedPacket:address:)] ) {
                [strongDelegate simplePing:self didReceiveUnexpectedPacket:packet address: address];
            }
        }
    } else {
